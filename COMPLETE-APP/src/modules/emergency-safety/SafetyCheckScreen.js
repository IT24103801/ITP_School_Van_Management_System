...
export default function SafetyCheckScreen() {
  ...
  const renderCheck = ({ item }) => {
    const raw = item.checkItems ?? item.check_items;
    let itemsObj = {};
    ...
    const overall = item.overallStatus || item.overall_status || '';
    const statusKey = overall.replace(/\s+/g, '');
    ...
    <View style={styles.checkCard}>
      <View style={styles.header}>
        ...
        <Text style={[styles.status, styles[`status${statusKey}`] || {}]}>
          {overall}
        </Text>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => `${String(item.id ?? item.checkId ?? 'check')}-${index}`}
